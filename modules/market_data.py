...
def get_kr_etf_monthly_baseline(ticker: str, retry=3, delay=2) -> Optional[Dict]:
    """한국 ETF 전월 마지막 거래일 가격 조회 (ISA 트리거용)
    
    전월 말일 기준 → 이번 달 순수 등락률 계산
    """
    if not FDR_AVAILABLE:
        return None
    ...
    for attempt in range(retry):
        try:
            ...
            last_idx = prev_month_rows.index[-1]
            baseline_date = last_idx.strftime('%Y-%m-%d')
            baseline_price = prev_month_rows['Close'].iloc[-1]
            current_price = df['Close'].iloc[-1]
            change_pct = ((current_price - baseline_price) / baseline_price) * 100

            print(f"    📅 전월 말일 기준: {baseline_date}")
            time.sleep(2)
...
def get_kr_etf_multi_period_baselines(ticker: str, retry=3, delay=2) -> Optional[Dict]:
    ...
    for attempt in range(retry):
        try:
            ...
            def find_last_trading_day(year, month):
                """✅ 특정 연월의 마지막 거래일 가격 반환"""
                target_rows = df[(df.index.year == year) & (df.index.month == month)]
                if target_rows.empty:
                    return None, None
                last_date = target_rows.index[-1].strftime('%Y-%m-%d')
            ...
            for period_name, months_back in period_defs:
                ...
                date_str, price = find_last_trading_day(year, month)
...
def get_monthly_baseline_price(ticker: str, av_api_key: str, retry=3, delay=3) -> Optional[Dict]:
    """전월 마지막 거래일 가격 조회 - 개별주용 (Alpha Vantage)
    ...
    for attempt in range(retry):
        try:
            ...
            # ✅ 전월의 마지막 거래일 찾기 (내림차순 순회 → 전월 첫 번째 히트 = 말일)
            baseline_date = None
            baseline_price = None
            ...
            # current_price는 main.py에서 실시간 가격으로 덮어씀
...
def get_us_etf_multi_period_baselines(ticker: str, av_api_key: str, retry=3, delay=3) -> Optional[Dict]:
    """미국 지수 ETF 다기간 기준가 조회 (SPYM용, Alpha Vantage)
    ...
    for attempt in range(retry):
        try:
            ...
            def find_last_trading_day(year, month):
                """✅ 특정 연월의 마지막 거래일 가격 반환 (내림차순 순회 → 첫 히트 = 말일)"""
                for date_str in dates:
