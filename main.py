...
def main():
    ...
    for stock_config in config['watchlist']:
        ...
        if ticker.endswith('.KS') or ticker.endswith('.KRX'):
            ...
            if is_index:
                ...
                if multi_data:
                    ...
                    # ISA 트리거는 이번 달(monthly) 기준으로 체크
                    if stock_config.get('monthly_trigger') and multi_data.get('periods', {}).get('monthly'):
                        change_pct = multi_data['periods']['monthly']['change_pct']
                        if change_pct <= -10:
                            isa_trigger_data = {
                                'ticker': ticker,
                                'change_pct': change_pct,
                                'baseline_date': multi_data['periods']['monthly']['date'],
                                'baseline_price': multi_data['periods']['monthly']['price'],
                    ...
                    print(f"    📊 전월:{m['change_pct']:+.2f}% | 3M:{m3['change_pct']:+.2f}% | "
                          ...
                          if m and m3 and m6 and y1 else "    ⚠️  일부 기간 데이터 없음")
            else:
                # 개별주: 전월 1일 baseline만 (한국 개별주는 현재 없으나 확장성 확보)
        ...
        # ── 미국 주식 ────────────────────────────────────────────
            ...
            if is_index:
                # 지수 ETF(SPYM): 다기간 baseline (full outputsize, 1 API call)
